...
async def cevir(event):
    botcilik = event.pattern_match.group(1)
    try:
        if len(botcilik) < 1:
            await edit_delete(
                event,
...
async def makevoice(event):
    ...
    await runcmd(
        f"ffmpeg -i '{file}' -map 0:a -codec:a libopus -b:a 100k -vbr on cilik.opus"
    )
    await event.client.send_message(
        event.chat_id, file="cilik.opus", force_document=False, reply_to=msg
    )
    await xxnx.delete()
    os.remove(file)
    os.remove("cilik.opus")
